<x-app-layout>
    <div class="container mx-auto px-4 py-2 flex flex-col justify-center">
        <form class="flex flex-col items-center" method="POST" action="{{ route('posts.update', $post->id) }}">
            @csrf
    
            <div class="w-1/2 mt-4">
                <x-input-label for="title" class="text-center" :value="__('Title')" />
                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{$post->title}}" placeholder="Enter your title here..." required autocomplete="title" />
            </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6 flex items-center">
                        <textarea 
                            ...
                            name="content"
                            required autocomplete="content">{{ $post->content }}</textarea>
                        @error('content')
